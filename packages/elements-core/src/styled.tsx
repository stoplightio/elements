...
export class Styled extends React.Component<React.PropsWithChildren<unknown>> {
...
export function withStyles<T extends React.JSX.IntrinsicAttributes>(
  Component: React.ComponentType<React.PropsWithChildren<T>>,
): React.FC<React.PropsWithChildren<T>> {
  const Inner: React.FC<React.PropsWithChildren<T>> = props => {
    return (
      <Styled>
        <Component {...props} />
      </Styled>
