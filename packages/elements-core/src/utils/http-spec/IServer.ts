...
export const getServersToDisplay = (
  ...
  const servers = originalServers
    .map<ServerWithOptionalUrl>((server, i) => {
      ...
      return {
        ...server,
        url: getServerUrlWithFilledVariables(server, variables),
...
export const getServerUrlWithFilledVariables = (server: IServer, values?: { [key: string]: string }): string | null => {
  let urlString = server.url;

  const variables = Object.entries(server.variables ?? {});
  variables.forEach(([variableName, variableInfo]) => {
    urlString = urlString.replace(`{${variableName}}`, values?.[variableName] || variableInfo.default);
  });
