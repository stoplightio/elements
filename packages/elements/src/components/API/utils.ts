...
export const computeTagGroups = (serviceNode: ServiceNode, childNodes: ServiceChildNode[]) => {
...
export const computeAPITree = (serviceNode: ServiceNode, childNodes: ServiceChildNode[]) => {
  ...
  const operationNodes = childNodes.filter(node => node.type === NodeType.HttpOperation);
  if (operationNodes.length) {
    tree.push({
      title: 'Endpoints',
    });

    const { groups, ungrouped } = computeTagGroups(serviceNode, childNodes);
  ...
  const schemaNodes = childNodes.filter(node => node.type === NodeType.Model);
