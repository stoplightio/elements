...
import { Code, FAIcon } from '@stoplight/ui-kit';
  ...
  useEffect(() => {
    const headerElement = (
      <strong className="mb-1">
        ...
        {errorCount > 0 && (
          <span className="pr-3">
            <FAIcon icon={faExclamationCircle.iconName} className="mr-2" />
            <span>{errorCount}</span>
          </span>
        )}
        {warningCount > 0 && (
          <span className="pr-3">
            <FAIcon icon={faExclamationTriangle.iconName} className="mr-1" />
