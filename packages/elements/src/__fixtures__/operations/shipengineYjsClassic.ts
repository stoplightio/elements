...
const makeOperation: () => YifyClassic<WithIds<IHttpOperation>> = () =>
  YjsifyClassic<WithIds<IHttpOperation>>({
    id: '1',
    iid: 'parse_address',
    description:
...
export const resetOperation = () => {
  ...
  ydoc.doc.getMap('root').set('operation-classic', httpOperation);
  console.log('httpOperation', httpOperation);
};

ydoc.ready.then(() => {
  if (!ydoc.doc.getMap('root').has('operation-classic')) {
