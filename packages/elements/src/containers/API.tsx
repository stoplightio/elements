...
const APIImpl: React.FC<APIProps> = props => {
  ...
  const oasNodes = React.useMemo(() => computeOasNodes(bundledDocument), [bundledDocument]);
  ...
  if (!oasNodes || !oasNodes.serviceNode || !oasNodes.childNodes?.length) {
    return (
      <Flex justify="center" alignItems="center" w="full" minH="screen">
        <NonIdealState
  ...
  return (
    <InlineRefResolverProvider document={document}>
      {layout === 'stacked' ? (
        ...
        <SidebarLayout serviceNode={oasNodes.serviceNode} childNodes={oasNodes.childNodes} />
      )}
